...
/**
 * Test h2 client with prior on and off for service unavailable scenario.
 */
public class ConnectionRefusedTestCase {

    private static final Logger LOG = LoggerFactory.getLogger(ConnectionRefusedTestCase.class);

    private HttpClientConnector h2PriorOnClient, h2PriorOffClient;
    ...
    @Test
    public void testConnectionRefuseWithPriorOn() {
        try {
            HttpCarbonMessage httpsRequest = MessageGenerator.generateRequest(HttpMethod.GET, "",
                                                                              SERVER_CONNECTOR_PORT, "http://");

            CountDownLatch latch = new CountDownLatch(1);
            DefaultHttpConnectorListener listener = new DefaultHttpConnectorListener(latch);
            HttpResponseFuture responseFuture = h2PriorOnClient.send(httpsRequest);
    ...
    @Test
    public void testConnectionRefuseWithPriorOff() {
        try {
            HttpCarbonMessage httpsRequest = MessageGenerator.generateRequest(HttpMethod.GET, "",
                                                                              SERVER_CONNECTOR_PORT, "http://");
            ...
            CountDownLatch latch = new CountDownLatch(1);
            DefaultHttpConnectorListener listener = new DefaultHttpConnectorListener(latch);
            HttpResponseFuture responseFuture = h2PriorOffClient.send(httpsRequest);
            responseFuture.setHttpConnectorListener(listener);
